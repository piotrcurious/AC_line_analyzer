  ...
  if (poll_pll()){
    ...
    if (phase_est.estimate_phase(pe_result)) {
      ...
      switch(pe_result.state) {
        case PE_STABLE:
          // Stable linear drift - apply both phase and frequency correction
          phase_err_rad = -pe_result.linear_drift_rate * 0.8f; // Gentle phase correction
          
          // Apply frequency correction if estimate is valid
          if (freq_valid && freq_result.confidence > 0.5f) {
            freq_correction_hz = freq_result.pll_correction_hz * 0.1f; // Small incremental correction
          }
          apply_correction = true;
          break;
          
        case PE_CORRECTION_DETECTED:
          // We're tracking a correction we applied
          if (pe_result.correction_applied) {
            // Check if it's effective
            if (pe_result.correction_effective) {
              Serial.printf("✓ Correction EFFECTIVE: %.3f rad (%.1f deg) - New trend established\n",
                           pe_result.correction_magnitude,
                           pe_result.correction_magnitude * 180.0f / M_PI);
            } else {
              Serial.printf("⧗ Correction tracking: %.3f rad (%.1f deg) - Waiting for trend...\n",
                           pe_result.correction_magnitude,
                           pe_result.correction_magnitude * 180.0f / M_PI);
            }
          } else {
            // Unexpected correction detected
            Serial.printf("! UNEXPECTED correction: %.3f rad (%.1f deg)\n",
                         pe_result.correction_magnitude,
                         pe_result.correction_magnitude * 180.0f / M_PI);
          }
          // Don't apply any correction while tracking
          phase_err_rad = 0.0f;
          freq_correction_hz = 0.0f;
          apply_correction = false;
          break;
          
        case PE_NONLINEAR_DRIFT:
          // Non-linear changes (signal variations) - be conservative
          // Only correct phase, no frequency adjustment
          if (fabs(pe_result.recent_phase_shift) > 0.05f) {
            phase_err_rad = -pe_result.recent_phase_shift * 0.3f; // Very gentle
          }
          apply_correction = (fabs(phase_err_rad) > 0.01f);
          break;
      ...
      if (frame_counter % 10 == 0) {
        ...
        if (pe_result.frequency_estimate_valid) {
          Serial.printf("Freq Est: %.4f Hz (error: %+.4f Hz, conf: %.2f)\n",
                       pe_result.estimated_frequency,
                       pe_result.estimated_frequency_error,
                       freq_result.confidence);
        }
        
        Serial.printf("Correction Applied: %s", pe_result.correction_applied ? "YES" : "NO");
        if (pe_result.correction_applied) {
          Serial.printf(" | Effective: %s (%.3f rad / %.1f deg)", 
                       pe_result.correction_effective ? "YES" : "PENDING",
                       pe_result.correction_magnitude,
                       pe_result.correction_magnitude * 180.0f / M_PI);
        }
        Serial.println();
    ...
    // Apply frequency correction if recommended
    if (apply_correction && fabs(freq_correction_hz) > 0.001f) {
      float new_freq = sys.grid_f + freq_correction_hz;
      // Clamp to reasonable range
      if (new_freq > NOMINAL_FREQ - 5.0f && new_freq < NOMINAL_FREQ + 5.0f) {
        tune_pll(new_freq);
        Serial.printf("Applied frequency correction: %+.4f Hz -> %.4f Hz\n",
                     freq_correction_hz, new_freq);
      }
    }
    
    // Apply phase correction if needed
    if (apply_correction && fabs(phase_err_rad) > 1e-6f) {
      apply_phase_rad(phase_err_rad);
      
      // Notify phase estimator that we applied a correction
      phase_est.notify_correction_applied(phase_err_rad);
    ...
    Serial.printf("F:%.4fHz | StrobeJit:%d cyc (%.2fus) | Samp0Jit:%d cyc | State:%s\n",
      sys.grid_f, sys.strobe_offset_cycles, dt_strobe_s*1e6, sys.buf[0].jit,
