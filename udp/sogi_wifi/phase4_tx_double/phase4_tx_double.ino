// too slow in double. need to be optimized 
...
uint16_t global_packet_index = 0;
...
static inline void IRAM_ATTR processIncomingDMA() {
        ...
        for (int pair = 0; pair < total_pairs; ++pair) {
            ...
            while (signed_time_diff(ts, next_sample_int) >= 0) {
                ...
                if (resamp.acc_weight > 0.0) {
                    ...
                    double u = v_val - v_dc_offset;
                    // Full Harmonic Decoupling: Fundamental - 3rd - 5th - 7th
                    double v_sum_harmonics = sogi_v3.v_alpha + sogi_v5.v_alpha + sogi_v7.v_alpha + sogi_v9.v_alpha + sogi_v11.v_alpha;
                    double u1 = u - v_sum_harmonics;
                    ...
                    double ts_v = ((double)ticks_per_sample_int + ticks_per_sample_frac) * inv_cpu_freq;
                    ...
                    double fll_err = sogi_v1.getFllError(u1);
                    double rot_err = (sogi_v1.getRotationRate() - fll.omega) / (fll.omega + 1.0f);
...
void IRAM_ATTR visTask(void *pv) {
    int idx;
    VisPacket packet;
    for (;;) {
        if (xQueueReceive(visQueue, &idx, portMAX_DELAY) == pdTRUE) {
            VisSnapshot &s = vis_snapshots[idx];

            packet.freq = s.pll_freq;
            packet.index = global_packet_index++;

            // Resample to 128 points, centered at DC
            for (int x = 0; x < 128; x++) {
                int sample_idx = (s.aligned_start + (x * s.count / 128)) % s.count;
                packet.v[x] = (int16_t)(s.v_copy[sample_idx] - s.vdc);
                packet.i[x] = (int16_t)(s.i_copy[sample_idx] - s.idc);
            }
...
#ifdef LOCAL_VISUALIZATION
            vis.update(s.v_copy, s.i_copy, s.count, s.aligned_start, s.count, s.pll_freq, 0.0f, s.vdc, s.idc);
#endif // LOCAL_VISUALIZATION
...
void setup() {
    ...
    // we block because without connection the system will crash. no idea why . 
    ...
    xTaskCreatePinnedToCore(visTask, "vis", 8192, NULL, 2, &visTaskHandle, 0);
    #ifdef LOCAL_LOGGING
    xTaskCreatePinnedToCore(logTask, "log", 4096, NULL, 1, &logTaskHandle, 0);
    #endif // LOCAL_LOGGING
}
