...
void IRAM_ATTR onTimer() {
  ...
  // Normalize phase to [0, 2*PI) range using modulo (more efficient)
  if (pll_phase >= PI_2) {
  ...
  // Cache phase for output calculation
  double current_phase = pll_phase;
  
  portEXIT_CRITICAL_ISR(&timerMux);

  // 2. GENERATE OUTPUT (Locked Sine Wave)
  // Output a clean sine wave based on internal LOCKED phase
  float out_val = OUTPUT_AMPLITUDE * sin(current_phase);
...
void setup() {
  ...
  Serial.println("Format: Freq(Hz) | Phase_Err(deg) | RMS(V) | Magnitude(V) | ISR(us)");
  Serial.println("-------------------------------------------------------------------");
}
