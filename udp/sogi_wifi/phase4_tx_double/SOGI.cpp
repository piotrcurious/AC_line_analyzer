...
void IRAM_ATTR SOGI::step(float input, float omega, float ts) {
    ...
    if (mag_sq > 100.0f) {
        double phi_now = atan2f(v_alpha, -v_beta);
        if (!has_prev_phi) { w_rot = omega; has_prev_phi = true; }
        else {
            double phi_pre = atan2f(v_alpha_prev, -v_beta_prev);
            double d_phi = phi_now - phi_pre;
            while (d_phi > (double)M_PI) d_phi -= 2.0f * (double)M_PI;
            while (d_phi < -(double)M_PI) d_phi += 2.0f * (double)M_PI;
...
double IRAM_ATTR SOGI::getFllError(double input) const {
    //double mag_sq = v_alpha * v_alpha + v_beta * v_beta + MAG_EPS; //TODO : this is too slow
    float mag_sq = v_alpha * v_alpha + v_beta * v_beta + MAG_EPS; //TODO : this is too slow
...
inline void IRAM_ATTR AdaptiveFLL::setDistortionDamping(float p_scale, float learn_scale) {
...
void IRAM_ATTR AdaptiveFLL::update(double fll_err, double rot_err, double ts) {
    // Both inputs are frequency deviations (positive = input is faster)
    double diff = fll_err - rot_err;
    // Refined agreement logic: sharp rejection of transients
    double agree = 1.0f / (1.0f + 100.0f * diff * diff);
    ...
    double fused_err = fll_err * agree + rot_err * (1.0f - agree);
    ...
    double d_omega = gamma * fused_err * ts;

    // Kahan summation frequency integrator
    double y = d_omega - integral_err_c;
    double t = omega + y;
