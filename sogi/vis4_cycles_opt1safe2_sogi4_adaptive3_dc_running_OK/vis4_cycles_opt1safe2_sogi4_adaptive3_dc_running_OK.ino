    ...
    if (elapsed_cycle >= single_cycle_cycles) {
        ...
        if (prev_cycle == 2) {
            ...
            if (actual_count > 0) {
                ...
                // --- CORRECTED PHASE ALIGNMENT & PERIOD CALCULATION ---

                // Calculate current sampling interval (dt)
                float current_dt = (float)sogi.ticks_per_sample * sogi.inv_cpu_freq;
                
                // 1. Calculate how many samples actually constitute one full period 
                // based on the frequency the SOGI has just calculated.
                // Formula: Samples = (Sample Frequency) / (Signal Frequency)
                // Note: sogi.sample_freq is 1.0f / sogi.dt
                //float samples_per_actual_period = (1.0f / current_dt) / sogi.freq;
                float samples_per_actual_period = 1.0f / (sogi.freq * current_dt);

                // 2. Phase alignment for Visualizer
                float end_phase = atan2f(sogi.v_beta, sogi.v_alpha);
                float target_phase = -PI / 2.0f; // Align to negative peak for visual stability
                float phase_diff = end_phase - target_phase;
                
                // Wrap phase difference to [0, 2PI)
                while (phase_diff < 0) phase_diff += 2.0f * PI;
                while (phase_diff >= 2.0f * PI) phase_diff -= 2.0f * PI;

                // 3. Use the dynamic 'samples_per_actual_period' instead of SAMPLES_PER_CYCLE
                ...
                // 4. Update visualizer with the actual period length
                // This ensures the visualizer shows exactly one full wave.

                // something is off though as those numbers differ (actual count and our calculations)
                ...
                Serial.printf("F:%.5fHz, Mag:%.3f, Err:%.3f, ActSamples:%d\n", 
