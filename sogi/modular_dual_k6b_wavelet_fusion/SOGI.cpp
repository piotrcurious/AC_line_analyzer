...
void AdaptivePLL::init() {
    ...
    // State: [Theta_sig (rad), Omega_sig_dev (rad/s), SOGI_Bias (rad)] - Relative to 50Hz
    x_theta = 0; x_omega = 0; x_beta = 0;

    memset(P, 0, sizeof(P));
    P[0][0] = 0.1f; P[1][1] = 1.0f; P[2][2] = 0.1f;
    memset(Q, 0, sizeof(Q));
    Q[0][0] = 0.01f; Q[1][1] = 0.1f; Q[2][2] = 0.001f;
}
...
void AdaptivePLL::sequentialUpdate(const float z[], const float h[], const float R[], const float H[][3], int num_measurements) {
    for (int i = 0; i < num_measurements; i++) {
        ...
        if (fabs(S) > 1e-12f) {
            ...
            float innov = z[i] - h[i];
            // Normalize innovations for phase measurements (z0 is SOGI phase error)
            if (i == 0) {
...
void AdaptivePLL::update(float v_alpha, float v_beta, float ts) {
    ...
    if (mag_smooth > 0.10f) {
        ...
        predict(ts);
        // This 'update' is usually called per-sample or per-window without wavelet.
        // We assume pll_phi_dev is 0 or handled externally.
        // For simplicity, treat x_theta as the error signal.
...
void AdaptivePLL::updateFused(float v_alpha, float v_beta, float wavelet_freq, float pll_phi_dev, float confidence, float dt) {
    float mag_inst = sqrtf(v_alpha * v_alpha + v_beta * v_beta);
    mag_smooth = (MAG_ALPHA * mag_inst) + ((1.0f - MAG_ALPHA) * mag_smooth);
    if (mag_smooth > 0.10f) {
        predict(dt);

        float z[2] = { v_beta / mag_smooth, (wavelet_freq - nominal_freq) * 2.0f * PI };
        // z0 = SOGI error = Theta_sig - Theta_pll + Bias
        // h0 = (Theta_sig - Theta_50) - (Theta_pll - Theta_50) + Bias
        // h0 = x_theta - pll_phi_dev + x_beta
        float h[2] = { (x_theta - pll_phi_dev) + x_beta, x_omega };
        float R[2] = { 0.1f, 0.01f / (confidence + 0.01f) };
        float H[2][3] = { { 1.0f, 0.0f, 1.0f }, { 0.0f, 1.0f, 0.0f } };

        sequentialUpdate(z, h, R, H, 2);

        // Control: Frequency = Nominal + Signal_Dev + Phase_Correction
        // Phase correction is based on the error between signal and current PLL
        float phase_err = x_theta - pll_phi_dev;
        while (phase_err > PI) phase_err -= 2.0f * PI;
        while (phase_err < -PI) phase_err += 2.0f * PI;

        freq = nominal_freq + (x_omega / (2.0f * PI)) + (kp * phase_err);
        omega = 2.0f * PI * freq;
    }
}
