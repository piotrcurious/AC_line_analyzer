...
// Define the number of samples to analyze the harmonic content
#define HARMONIC_WINDOW 10

// Define the exponential function parameters
// f(x) = a * exp(b * x)
#define a 1
#define b 0.1
...
// Define a semaphore to synchronize the tasks
SemaphoreHandle_t sem = NULL;

// Define a task handle to create the harmonic analysis task
TaskHandle_t harmonicTask = NULL;

// Define a function to analyze the harmonic content of the signal
void analyzeHarmonic(void * parameter) {
  // Create a local buffer to store the samples
  int harmonic_buffer[HARMONIC_WINDOW];

  // Create a local variable to store the index of the buffer
  int harmonic_index = 0;

  // Create a local variable to store the sum of the samples
  int harmonic_sum = 0;

  // Create a local variable to store the average of the samples
  float harmonic_avg = 0;

  // Create a local variable to store the standard deviation of the samples
  float harmonic_std = 0;

  // Create a local variable to store the exponential function value
  float harmonic_exp = 0;

  // Loop forever
  for (;;) {
    // Wait for the semaphore to be given by the zero crossing task
    if (xSemaphoreTake(sem, portMAX_DELAY) == pdTRUE) {
      // Read the current sample from the analog pin
      int curr_sample = analogRead(AC_PIN);

      // Store the sample in the buffer
      harmonic_buffer[harmonic_index] = curr_sample;

      // Update the buffer index
      harmonic_index = (harmonic_index + 1) % HARMONIC_WINDOW;

      // Check if the buffer is full
      if (harmonic_index == 0) {
        // Reset the sum of the samples
        harmonic_sum = 0;

        // Calculate the sum of the samples
        for (int i = 0; i < HARMONIC_WINDOW; i++) {
          harmonic_sum += harmonic_buffer[i];
        }

        // Calculate the average of the samples
        harmonic_avg = harmonic_sum / (float) HARMONIC_WINDOW;

        // Reset the standard deviation of the samples
        harmonic_std = 0;

        // Calculate the standard deviation of the samples
        for (int i = 0; i < HARMONIC_WINDOW; i++) {
          harmonic_std += pow(harmonic_buffer[i] - harmonic_avg, 2);
        }
        harmonic_std = sqrt(harmonic_std / (float) HARMONIC_WINDOW);

        // Calculate the exponential function value
        harmonic_exp = a * exp(b * harmonic_std);

        // Print the harmonic content to the serial monitor
        Serial.print("Harmonic content: ");
        Serial.println(harmonic_exp);
      }
    }
  }
}

// Define a function to detect the zero crossing of the signal
void detectZeroCrossing(void * parameter) {
  // Loop forever
  for (;;) {
    // Read the current sample from the analog pin
    int curr_sample = analogRead(AC_PIN);
      ...
      if ((min_sample < AC_OFFSET * ADC_RES / ADC_REF && max_sample > AC_OFFSET * ADC_RES / ADC_REF)) {
        ...
        if (frequency >= AC_MIN_FREQ && frequency <= AC_MAX_FREQ) {
          ...
          if (amplitude >= AC_MIN_AMP && amplitude <= AC_MAX_AMP) {
            ...
            // Give the semaphore to the harmonic analysis task
            xSemaphoreGive(sem);
          }
        }
...
void setup() {
  ...
  // Create a task for the harmonic analysis function
  xTaskCreate(
    analyzeHarmonic, // Function name
    "Harmonic Analysis", // Task name
    1000, // Stack size
    NULL, // Parameter
    1, // Priority
    &harmonicTask // Task handle
  );

  // Call the zero crossing detection function
  detectZeroCrossing(NULL);
}
