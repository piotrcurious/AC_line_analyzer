...
float PhaseEstimator::compute_phase_shift(const uint16_t* reference, const uint16_t* target) {
  ...
  for (uint16_t i = 0; i < PE_SAMPLES_PER_CYCLE; i++) {
    float d = ref_cycle[i] - ref_mean;
    correlation_buffer[i] = d;
    ref_std += d * d;
  }
  ref_std = sqrtf(ref_std / PE_SAMPLES_PER_CYCLE);
  if (ref_std < 1.0f) ref_std = 1.0f;
  float inv_ref_std = 1.0f / ref_std;
  for (uint16_t i = 0; i < PE_SAMPLES_PER_CYCLE; i++) correlation_buffer[i] *= inv_ref_std;

  float min_residual = 1e9f;
  int16_t best_offset = 0;

  // Hierarchical Search (Stride 8 then 1)
  for (int offset = 0; offset < 128; offset += 8) {
      float sum = 0;
      for (int i=0; i<128; i+=2) {
          int tidx = (i + offset) & 127;
          float tval = (search_cycle[tidx] - ref_mean) * inv_ref_std;
          float d = correlation_buffer[i] - tval;
          sum += d*d;
      }
      if (sum < min_residual) { min_residual = sum; best_offset = offset; }
  }

  int coarse_best = best_offset;
  min_residual = 1e9f;
  for (int offset = coarse_best - 7; offset <= coarse_best + 7; offset++) {
      int a_off = (offset + 128) & 127;
      float sum = 0;
      for (int i=0; i<128; i++) {
          int tidx = (i + a_off) & 127;
          float tval = (search_cycle[tidx] - ref_mean) * inv_ref_std;
          float d = correlation_buffer[i] - tval;
          sum += d*d;
      }
      residual_buffer[a_off] = sum;
      if (sum < min_residual) { min_residual = sum; best_offset = a_off; }
  }

  float sub_offset = (float)best_offset;
  float r_prev = residual_buffer[(best_offset - 1 + 128) & 127];
  float r_curr = residual_buffer[best_offset];
  float r_next = residual_buffer[(best_offset + 1) & 127];
  float denom = r_prev - 2.0f * r_curr + r_next;
  if (fabs(denom) > 1e-6f) {
      sub_offset += 0.5f * (r_prev - r_next) / denom;
  }

  float phase_rad = -(2.0f * M_PI * sub_offset) / 128.0f;
...
bool PhaseEstimator::estimate_phase(PhaseEstResult& result) {
  ...
  // Optimization: Only compute the trend point for the NEWEST frame relative to anchor
  // To keep it compatible with existing analyze_trend, we still build the trend,
  // but we should cache the intermediate results.
  // For now, let's just compute the trend points. It's only 15 calls.
  // With hierarchical search, it's fast enough.
