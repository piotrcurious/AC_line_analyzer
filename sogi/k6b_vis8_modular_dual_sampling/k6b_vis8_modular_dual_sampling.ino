...
void loop() {
    ...
    if (elapsed_sample >= ticks_per_sample && current_cycle < 3) {
        ...
        v_samp_buf[buf_idx] = (float)raw_v; // k6b used raw values in buf
        i_samp_buf[buf_idx] = (float)raw_i;
    ...
    if (elapsed_cycle >= single_cycle_cycles) {
        ...
        if (prev_cycle == 2) {
            ...
            if (actual_count > 0) {
                ...
                Serial.printf("F:%.4fHz, Mag:%.3f, Vdc:%.1f, Idc:%.1f, Core:%.1fus, Vis:%.1fus, GainEst:%.4f\n",
                              pll.freq, pll.mag_smooth, v_dc_offset, i_dc_offset, core_us, vis_us, pll.gain_est);
            }
        }
    }
}
