...
static inline uint32_t get_cycle_count() { uint32_t ccount; asm volatile("rsr %0, ccount" : "=a" (ccount)); return ccount; }
...
void updateTimingParameters(float frequency) {
  ...
  if (old_ticks > 0 && old_ticks != 0) {
    // Preserve sampling phase when ticks_per_sample changes:
    // compute the current position within the old sample period (sample_phase)
    // and map it to the new ticks_per_sample to keep sampling aligned.
    uint32_t elapsed = now - last_sample_cycles;
    uint32_t sample_phase = elapsed % old_ticks;
    last_sample_cycles = now - (uint32_t)((float)sample_phase * (float)sogi.ticks_per_sample / (float)old_ticks);
  }
}
...
void loop() {
  ...
  if (sogi.ticks_per_sample > 0 && elapsed_sample >= sogi.ticks_per_sample && current_cycle < 3) {
    last_sample_cycles += sogi.ticks_per_sample;

    int raw = analogRead(ADC_PIN);

    dc_current = (DC_ALPHA * (float)raw) + ((1.0f - DC_ALPHA) * dc_current);
    // Use the processing DC offset (stable during sampling phase)
  ...
  if (single_cycle_cycles > 0 && elapsed_cycle >= single_cycle_cycles) {
    ...
    if (prev_cycle == 2) {
      ...
      if (actual_count > 0) {
        // Update sampling DC offset for next cycle (no feedback during current sampling)
        ...
        int samples_back = (int)((phase_diff / (2.0f * PI)) * (float)SAMPLES_PER_CYCLE);
        int processed_end_idx = (e_idx - 1 + BUF_N) % BUF_N;
        int aligned_start_idx = (processed_end_idx - samples_back + BUF_N) % BUF_N;

        // Restored debug output and fixed visualizer update count
        vis.update(samp_buf, BUF_N, aligned_start_idx, SAMPLES_PER_CYCLE,
                   sogi.freq, sogi.mag_smooth, sogi.filtered_err);

        // Print absolute frequency and offset (keeps previous display info, clearer)
        Serial.printf("F:%.5fHz (off %.5fHz), Mag:%.3f, Err:%.3f\n",
                      sogi.freq, (sogi.freq - NOMINAL_FREQ), sogi.mag_smooth, sogi.filtered_err);
      }
    }
  }
}
