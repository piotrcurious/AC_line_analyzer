...
// + integer triangular dither applied to remainder accumulator to decorrelate quantization
...
// DITHER: triangular integer dither width in bits (0 = disabled).
// Typical: 3..6. Larger = more noise, fewer discrete tones.
const int DITHER_BITS = 5;
const uint32_t DITHER_MASK = (DITHER_BITS > 0) ? ((1u << DITHER_BITS) - 1u) : 0u;
// -----------------------------------------
...
static inline int16_t trapezoidFromPhaseIdxAndErr(uint32_t idx, uint64_t err, uint64_t denom) {
  // We compute position p in [0,1) using double here — trapezoid is a rare mode.
  double phase_frac = ((double)idx + (double)err / (double)denom) / (double)WAVETABLE_SIZE;
  double p = fmod(phase_frac, 1.0);
  float out;
  if (p < 0.20f) {
    float t = p / 0.20f;
    out = -1.0f + 2.0f * t;
...
// streaming task: pure integer Bresenham stepping + integer triangular dither
void i2s_stream_task(void *arg) {
  Serial.println("Stream task started (pure-integer Bresenham + integer dither)");
  ...
  while (1) {
    ...
    // compute integer numerator step using micro-Hz scale (one division per chunk)
    ...
    for (int n = 0; n < CHUNK_FRAMES; ++n) {
      ...
      if (mode == WM_SINE) {
        // Basic integer index increment
        phase_idx += (uint32_t)q;

        // Add remainder then apply integer triangular dither when testing overflow
        err += rem;

        if (DITHER_BITS > 0) {
          // triangular: (r1 & mask) - (r2 & mask)
          uint32_t r1 = xorshift32();
          uint32_t r2 = xorshift32();
          int32_t tri = (int32_t)((r1 & DITHER_MASK) - (r2 & DITHER_MASK));
          int64_t cmp = (int64_t)err + (int64_t)tri;
          if (cmp >= (int64_t)denom) {
            phase_idx++;
            // set err to cmp - denom (keep remainder including dither residual)
            err = (uint64_t)(cmp - (int64_t)denom);
          } else if (cmp < 0) {
            // clamp negative comparison to 0 remainder (safe)
            // we intentionally do not underflow accumulator; clamp to 0.
            err = 0;
          } else {
            // keep the adjusted remainder (gives dithered fractional position)
            err = (uint64_t)cmp;
          }
        } else {
          // no dither: normal Bresenham carry
          if (err >= denom) {
            phase_idx++;
            err -= denom;
          }
        ...
        uint64_t tmp = err * 65536ULL; // fits in 64-bit for denom ~= 4.8e10
        uint32_t frac16 = (uint32_t)(tmp / denom);
        ...
        // trapezoid: fallback to double eval (rare)
        sample_int16 = (int32_t)trapezoidFromPhaseIdxAndErr(phase_idx, err, denom);

        // advance with same integer step rules (to keep phasing consistent)
        phase_idx += (uint32_t)q;
        err += rem;
        if (DITHER_BITS > 0) {
          uint32_t r1 = xorshift32();
...
// Run one test pattern (same as before)
...
void setup() {
  Serial.begin(115200);
  delay(10);
  Serial.println("\nESP32 I2S->DAC pure-integer Bresenham + integer dither");
  ...
  Serial.print("DITHER_BITS: "); Serial.println(DITHER_BITS);
