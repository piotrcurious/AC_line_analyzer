...
void logTask(void *pv) {
    ...
    for (;;) {
        if (xQueueReceive(logQueue, &msg, portMAX_DELAY) == pdTRUE) {
            Serial.printf(
                "F:%.4fHz,Core:%.1fus,Drop:%lu,Ipol:%lu/%lu,Vdc:%.1f,Idc:%.1f,Ratio:%.3f\n",
...
void IRAM_ATTR loop() {
   ...
   //     float v_avg = (v_sum / SAMPLES_PER_CYCLE);
   //     float i_avg = (i_sum / SAMPLES_PER_CYCLE);
    v_dc_offset += (v_avg - v_dc_offset) >>4 ;
    i_dc_offset += (i_avg - i_dc_offset) >>4 ;
   // v_dc_offset = v_avg  ;
   // i_dc_offset = i_avg  ;

    if (v_dc_offset < 200) { v_dc_offset += 200; }
    if (i_dc_offset < 200) { i_dc_offset += 200; }

    //if (v_dc_offset > 16384) { v_dc_offset = 16384; }
    ...
    float phase = analyzer.grid_phase + (2.0f * PI * analyzer.grid_freq) * lag_sec +0.25; // BUG: +0.25... but why?
