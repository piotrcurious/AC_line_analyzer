...
public:
    ...
    // Core EKF Fusion
    void updateFused(float v_alpha, float v_beta, float wavelet_freq, float pll_phi_dev, float confidence, float dt);
    ...
    // EKF State Vector: [Theta_sig (rad), Omega_sig_dev (rad/s), SOGI_Bias (rad)] - Relative to 50Hz
    float x_theta;
    float x_omega;
    float x_beta;
