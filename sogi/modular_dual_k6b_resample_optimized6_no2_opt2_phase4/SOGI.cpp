 ...
 *   FIXED: Frequency pre-warping is implemented to give exact frequency-domain
 *   matching at ω.
 *
 ...
 *   FIXED: They are implemented using a single shared Direct-Form II state to
 *   reduce arithmetic and storage.
 *
 *   OPTIMIZED: Tustin frequency pre-warping is used to ensure exact resonance.
 *
 ...
 *
 *   FIXED: in the anti-windup back-calculation, `integral_state = i_term / ki`
 *   is guarded against divide-by-zero when ki == 0.
 *
 * =============================================================================
 */
...
void SOGI::reset() {
    ...
    // Direct-Form II delay registers (shared for H_α and H_β)
    wz1 = wz2 = 0.0f;
}
...
//
// FIXED: Coefficients are automatically recomputed when omega or ts change
// significantly.
void IRAM_ATTR SOGI::updateCoefficients(float omega, float ts)
{
    ...
    // FIXED: implemented frequency pre-warping for the Tustin transform.
    // This ensures the digital resonator's peak is exactly at 'omega'.
    // wts_prewarped = 2 * tan(omega * ts / 2)
...
//
// Both sections share identical denominator coefficients, so they
// share a single state variable sequence w(k) in Direct-Form II:
//   w(k) = u(k) − a₁·w(k−1) − a₂·w(k−2)
//   y_α  = b₀_α·w(k) + b₂_α·w(k−2)
//   y_β  = b₀_β·w(k) + b₁_β·w(k−1) + b₂_β·w(k−2)
//
// FIXED: shared state variable used to reduce arithmetic and storage.
...
void AdaptivePLL::init()
{
    ...
    // from the harmonic detector in the main loop.
    // FIXED: added safe default initialisation.
    p_scale = 1.0f;
    learn_scale = 1.0f;
}
...
//
// FIXED (anti-windup back-calculation):
//   Guarded against divide-by-zero when ki == 0.
//
