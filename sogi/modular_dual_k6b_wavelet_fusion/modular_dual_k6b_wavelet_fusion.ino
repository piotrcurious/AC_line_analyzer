...
#define MAX_SEARCH 8
#define CLEANUP_FRAMES_DIVIDER 200 // cpu_freq/CLEANUP_FRAMES_DIVIDER to determine how old frames to keep . cannot be longer than duration of DMA frame and ts interval
...
void cleanupOldFrames(uint32_t current_time) {
    uint32_t keep_duration_cycles = cpu_freq_hz / CLEANUP_FRAMES_DIVIDER; //20 = 50ms this need to be larger if you keep large dma buffer
...
void setup() {
    ...
    if (phase_est.begin(&pe_config)) {
        // We will call add_frame every cycle. Strobe div is 1.0 cycles.
        phase_est.set_frequency_params(NOMINAL_FREQ, 1.0f/NOMINAL_FREQ, SAMPLES_PER_CYCLE, 1.0f, cpu_freq_hz);
        Serial.println("Phase estimator initialized.");
    }
        ...
        if (elapsed_cycle >= single_cycle_cycles) {
            ...
            if (prev_cycle == 2) { // process the window
                ...
                if (actual_count > 0) {
                    ...
                    const char* pe_state_str = "INI";
                    if (pe_valid) {
                        if (pe_result.state == PE_STABLE) pe_state_str = "STB";
                        else if (pe_result.state == PE_NONLINEAR_DRIFT) pe_state_str = "NLN";
                        else if (pe_result.state == PE_READY) pe_state_str = "RDY";
                    }

                    Serial.printf("F:%.4fHz, Mag:%.3f, Vdc:%.1f, Core:%.1fus, Jit:%.1fus, PE:%s, Drift:%.4f, Frames:%lu(%.0f%%), ISR:%lu, Interp:%lu/%lu, Drop:%lu\n",
                                  pll.freq, pll.mag_smooth, v_dc_offset, core_us,
                                  max_jitter_us, pe_state_str, pe_valid ? pe_result.linear_drift_rate : 0.0f,
                                  buf_frames, buf_util, isr_callback_count,
